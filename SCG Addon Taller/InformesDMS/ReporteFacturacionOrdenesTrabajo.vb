...
Public Class ReporteFacturacionOrdenesTrabajo : Implements IUsaMenu, IFormularioSBO, IUsaPermisos
...
#Region "Contructor"
    ...
    Public Sub New(ByVal ocompany As SAPbobsCOM.Company, _
                   ByVal SBOAplication As Application, ByVal p_menuInformesDMS As String, ByVal p_strUID_FORM_ReporteFacturacionOT As String)
        m_oCompany = ocompany
        m_SBO_Application = SBOAplication
        NombreXml = Windows.Forms.Application.StartupPath + My.Resources.Resource.XMLFormularioReporteFacturacionOrdenesTrabajo
        MenuPadre = p_menuInformesDMS
        Nombre = My.Resources.Resource.MenuReporteFacturacionInterna
        IdMenu = p_strUID_FORM_ReporteFacturacionOT
        Titulo = My.Resources.Resource.MenuReporteFacturacionInterna
        Posicion = 9
        FormType = p_strUID_FORM_ReporteFacturacionOT
...
#Region "Metodos"
    ...
    Private Sub CargarMatrixTipo()
        ...
        table.ExecuteQuery(" SELECT 'N' as sel, Name as des, Code as cod FROM [@SCGD_TIPO_ORDEN] WHERE U_Interna = 'N' OR U_INTERNA IS NULL ")
    ...
    Private Sub CargarCombo()
        ...
        Try
            sboItem = _FormularioSBO.Items.Item("cbo_Marca")
            ...
            Call Utilitarios.CargarValidValuesEnCombos(sboCombo.ValidValues, _
                                                        String.Format("SELECT Code,Name FROM [@SCGD_MARCA] WHERE U_UsaReporte = 'Y'"))
            sboCombo.ValidValues.Add(My.Resources.Resource.TODAS, My.Resources.Resource.TODAS)
            sboCombo.ValidValues.Add(My.Resources.Resource.OtrasMarcas, My.Resources.Resource.OtrasMarcas)
    ...
    Public Sub CargarParametros(ByVal formUID As String, ByVal pVal As SAPbouiCOM.ItemEvent, ByRef BubbleEvent As Boolean)
        ...
        Dim sucursales As String = " I.U_SCGD_idSucursal = '{0}' "
        Dim tiposOT As String = " I.U_SCGD_Tipo_OT = '{0}' "
        Dim strCodMarca As String = " I.U_SCGD_Cod_Marca = '{0}'"
        ...
        Dim dtConsulta As Data.DataTable
        ...
        sboItem = _FormularioSBO.Items.Item("cbo_Marca")
        sboCombo = DirectCast(sboItem.Specific, SAPbouiCOM.ComboBox)
        strMarca = sboCombo.Selected.Value
        strDescMarca = sboCombo.Selected.Description.Trim

        If strMarca = My.Resources.Resource.TODAS Then

            m_strConsulta = String.Format("SELECT CODE FROM [@SCGD_MARCA]")
            dtConsulta = Utilitarios.EjecutarConsultaDataTable(m_strConsulta)

            m_blnExisteAgregado = False

            For Each row As DataRow In dtConsulta.Rows
                If m_blnExisteAgregado Then
                    WhereMarca = WhereMarca + m_strOR
                End If
                WhereMarca = WhereMarca + String.Format(strCodMarca, row.Item("CODE").ToString())
                m_blnExisteAgregado = True
            Next

        ElseIf strMarca = My.Resources.Resource.OtrasMarcas Then

            m_strConsulta = String.Format("SELECT CODE FROM [@SCGD_MARCA] WHERE U_UsaReporte = 'N'")
            dtConsulta = Utilitarios.EjecutarConsultaDataTable(m_strConsulta)
        ...
        ElseIf Not String.IsNullOrEmpty(strMarca) Then
            WhereMarca = String.Format(strCodMarca, strMarca)
        End If

        parametros = String.Format("{0},{1},( {2} ),( {3} ),( {4} ), {5}", fechaF, fechaI, WhereSucursales, WhereTiposOT, WhereMarca, strDescMarca)

        If _rbtnResumido.ObtieneValorUserDataSource = "Y" Then
            Call Print(My.Resources.Resource.rptFacturacionOrdenesTrabajoResumido, My.Resources.Resource.TituloRPFacturacionOrdenesTrabajoResumido, parametros)
        Else
            Call Print(My.Resources.Resource.rptFacturacionOrdenesTrabajoDetallado, My.Resources.Resource.TituloRPFacturacionOrdenesTrabajoDetallado, parametros)
        End If
