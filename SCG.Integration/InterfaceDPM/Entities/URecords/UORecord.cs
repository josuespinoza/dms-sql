...
namespace SCG.Integration.InterfaceDPM.Entities.URecords
{
    public class UORecord
    {
        ...
        public String ShopNonStockedTotalHits { get; set; }
        public String ShopNonStocked1Pass { get; set; }
        public String ShopNonStockedLostSales { get; set; }
        ...
        public void ToString(ref StringBuilder p_sb)
        {
            ...
            try
            {
                ...
                p_sb.Append(ShopNonStockedTotalHits);
                p_sb.Append(ShopNonStocked1Pass);
                for (int i = 0; i < 5; i++)
                {
                    p_sb.Append(espacio);
                }
                p_sb.Append(ShopNonStockedLostSales);
                for (int i = 0; i < 1; i++)
                {
                    p_sb.Append(espacio);
                }
