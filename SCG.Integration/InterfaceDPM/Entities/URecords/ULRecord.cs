...
namespace SCG.Integration.InterfaceDPM.Entities.URecords
{
    public class ULRecord
    {
        ...
        public String CounterStockedTotalHits { get; set; }
        public String CounterStockedHits1Pass { get; set; }
        public String CounterStockedHitsLostSales { get; set; }
        ...
        public void ToString(ref StringBuilder p_sb)
        {
            ...
            try
            {
                ...
                p_sb.Append(CounterStockedTotalHits);
                p_sb.Append(CounterStockedHits1Pass);
                for (int i = 0; i < 5; i++)
                {
                    p_sb.Append(espacio);
                }
                p_sb.Append(CounterStockedHitsLostSales);
                for (int i = 0; i < 31; i++)
                {
                    p_sb.Append(espacio);
                }
